...
#include <glib.h>     //glib's hash tables
...
typedef struct AdjListNode {
  struct AdjList* dest;                // dest node pointer
  int             cost;
...
typedef struct AdjList {
  ...
  struct AdjList* pred;       // The index of the predecessor to this node, 
                              // initially NULL.
...
typedef struct Graph {
  ...
  GHashTable* array;     // array of adjacency lists
} Graph;

// function prototypes for functions defined in graph.c
Graph* buildGraph(char* source);
...
void freeGraph(Graph** graphAddr);
